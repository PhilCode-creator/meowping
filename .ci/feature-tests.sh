...
if ! echo "$OUTPUT1" | sed 's/\x1b\[[0-9;]*m//g' | grep -q "Cloudflare"; then
    echo "Test failed: Expected output to contain '1.1.1.1'"
    echo "Actual output:"
    echo "$OUTPUT1"
    exit 1
fi
...
if [[ "$HAVE_ICMP_PERMS" == "true" ]]; then
    if [[ -n "$USE_SUDO_ICMP" ]]; then
        OUTPUT3=$($USE_SUDO_ICMP $MEOWPING_PATH https://1.1.1.1 -c 1 -m)
    else
        OUTPUT3=$($MEOWPING_PATH https://1.1.1.1 -c 1 -m)
    fi
    if ! echo "$OUTPUT3" | sed 's/\x1b\[[0-9;]*m//g' | grep -q "Reply from"; then
        echo "Test failed: Expected output to contain 'Reply from' for https://1.1.1.1"
        echo "Actual output:"
        echo "$OUTPUT3"
        exit 1
    fi
